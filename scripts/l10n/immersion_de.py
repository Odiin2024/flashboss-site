# German strings for immersion.html.
# Canon sentences are LIFTED VERBATIM from the ratified German bundle copy
# (flashboss-admin/BUNDLE_COPY_ROOTS_SERIES_2026-09-19.md) wherever that sheet
# has them — the ladder heading, the two start sentences, the immersion law,
# the floor/ceiling phrasing and the four pack one-liners. The rest is mine.
# Pack names stay English. Numbers are the English page's, in German separators.
TITLE = "Das englische Immersionssystem — FlashBoss"
DESCRIPTION = ("Ein Weg, sechs Stufen: English Advance, German Roots, Norman Roots, English Adept, "
               "Latin Roots, Greek Roots. Englisch auf Englisch gelehrt, mit einem Bosskampf am Ende "
               "jedes Clusters.")

STRINGS = {
 # ---- nav / chrome ----
 ">immersion<": ">Immersion<",
 ">packs<": ">Packs<",
 ">word lists<": ">Wortlisten<",
 ">lessons<": ">Lektionen<",
 ">home<": ">Start<",
 ">voices<": ">Stimmen<",
 ">resources<": ">Ressourcen<",
 ">how it works<": ">so funktioniert's<",

 # ---- hero ----
 "English &middot;": "Englisch &middot;",
 "six steps": "sechs Stufen",
 "&middot; one path": "&middot; ein Weg",
 "The English immersion system": "Das englische Immersionssystem",
 "one path, six steps": "ein Weg, sechs Stufen",
 "Whether English is your first language or your second:":
   "Ob Englisch deine erste oder deine zweite Sprache ist:",
 "a six-step path through survival and out the other side of mastery.":
   "ein Weg in sechs Stufen, vom Überleben hindurch und auf der anderen Seite der Beherrschung wieder hinaus.",
 "Two steps ship free with the game. Four are the Roots packs. Every card is English, taught in English.":
   "Zwei Stufen liegen dem Spiel kostenlos bei. Vier sind die Roots-Pakete. Jede Karte ist Englisch, auf Englisch gelehrt.",
 "the packs": "die Packs",

 # ---- the ladder ----
 "The ladder": "Die Leiter",
 "One path, six steps": "Ein Weg, sechs Stufen",
 "The base game holds the floor and the ceiling of a normal immersion course. Two Roots packs sit inside it, two sit beyond it. Read the ladder top to bottom.":
   "Das Grundspiel trägt den Boden und die Decke eines normalen Immersionskurses. Zwei Roots-Pakete liegen darin, zwei darüber hinaus. Lies die Leiter von oben nach unten.",
 "Floor &mdash; base game": "Boden &mdash; Grundspiel",
 "Inside the course &mdash; DLC": "Im Kurs &mdash; DLC",
 "Ceiling &mdash; base game": "Decke &mdash; Grundspiel",
 "Beyond the course &mdash; DLC, elective": "Über den Kurs hinaus &mdash; DLC, freiwillig",
 "Optional": "Freiwillig",
 "Inside": "Im Kurs",
 "Ceiling": "Decke",
 "Beyond": "Darüber hinaus",

 "500 cards &middot; 500 words &middot; 25 clusters &middot; 15 lessons &middot; ships with the game":
   "500 Karten &middot; 500 Wörter &middot; 25 Cluster &middot; 15 Lektionen &middot; liegt dem Spiel bei",
 "The English of everyday tasks &mdash; forms, notices, instructions, offices. Read a form and fill it, follow written directions, hold the words that officialdom uses about you. The one optional door on the path.":
   "Das Englisch von Alltagsaufgaben &mdash; Formulare, Aushänge, Anleitungen, Ämter. Ein Formular lesen und ausfüllen, schriftlichen Anweisungen folgen, die Wörter parat haben, die die Behörde über dich benutzt. Die einzige freiwillige Tür auf dem Weg.",
 "1,000 cards &middot; 47 clusters &middot; 48 lessons":
   "1.000 Karten &middot; 47 Cluster &middot; 48 Lektionen",
 "The Germanic half of English: the plain words, the compounds whose seam has gone invisible, and the native prefixes &mdash;":
   "Die germanische Hälfte des Englischen: die schlichten Wörter, die Zusammensetzungen, deren Naht unsichtbar geworden ist, und die einheimischen Präfixe &mdash;",
 "1,000 cards &middot; 48 clusters &middot; 49 lessons":
   "1.000 Karten &middot; 48 Cluster &middot; 49 Lektionen",
 "The Old French layer that settled on top of the Germanic one. Every card names the twins:":
   "Die altfranzösische Schicht, die sich über die germanische gelegt hat. Jede Karte nennt die Zwillinge:",
 ", and the pack teaches the typical French spellings you learn to spot.":
   ", und das Paket lehrt die typischen französischen Schreibweisen, die du zu erkennen lernst.",
 "1,000 cards &middot; 56 clusters &middot; 12 lessons &middot; synonyms on all 1,000 cards &middot; ships with the game":
   "1.000 Karten &middot; 56 Cluster &middot; 12 Lektionen &middot; Synonyme auf allen 1.000 Karten &middot; liegt dem Spiel bei",
 "The articulate vocabulary of an adult reader, grouped by what it does. Read adult prose without stalling; have the word you actually meant, not the nearest one. Just to the end of Adept is a valuable resource for a high-schooler.":
   "Der gewandte Wortschatz eines erwachsenen Lesers, danach gruppiert, was er leistet. Erwachsene Prosa lesen, ohne zu stocken; das Wort haben, das du wirklich meintest, nicht das nächstbeste. Schon bis zum Ende von Adept ist das für eine Schülerin oder einen Schüler der Oberstufe ein wertvolles Werkzeug.",
 "500 cards = 1,000 words (a paired cousin on every card) &middot; 30 clusters &middot; 31 lessons":
   "500 Karten = 1.000 Wörter (auf jeder Karte ein verwandtes Partnerwort) &middot; 30 Cluster &middot; 31 Lektionen",
 "Famous Latin, everyday Latin, the prefixes, the great root families &mdash;":
   "Berühmtes Latein, alltägliches Latein, die Präfixe, die großen Wortfamilien &mdash;",
 "&mdash; and the suffixes.": "&mdash; und die Suffixe.",
 "teaches": "lehrt",
 "in the same breath.": "im selben Atemzug.",
 "1,000 cards &middot; 56 clusters &middot; 62 lessons &middot; the Greek script on all 1,000":
   "1.000 Karten &middot; 56 Cluster &middot; 62 Lektionen &middot; die griechische Schrift auf allen 1.000",
 "suffixes, hybrids flagged as hybrids. An unfamiliar technical word stops being unfamiliar.":
   "-Endungen, Hybride als Hybride gekennzeichnet. Ein unbekanntes Fachwort hört auf, unbekannt zu sein.",

 # ---- S4, verbatim from the ratified German bundle copy ----
 "Start at Advance if school let you down or English is new to you. Otherwise start at German Roots.":
   "Beginne mit Advance, wenn die Schule dich im Stich gelassen hat oder Englisch neu für dich ist. Ansonsten beginne mit German Roots.",
 "If your first language is a Romance one, take Norman Roots before German Roots.":
   "Ist deine Muttersprache eine romanische, nimm Norman Roots vor German Roots.",
 "The base game has the floor and the ceiling. German Roots and Norman Roots sit inside the course. Latin Roots and Greek Roots sit beyond it. English Advance is elective if you had a standard education. Latin Roots and Greek Roots are elective in full. There is no requirement to take the packs in order; the ladder is the recommendation.":
   "Das Grundspiel trägt den Boden und die Decke. German Roots und Norman Roots liegen im Kurs. Latin Roots und Greek Roots liegen darüber hinaus. English Advance ist freiwillig, wenn du eine normale Schulbildung hattest. Latin Roots und Greek Roots sind vollständig freiwillig. Es gibt keine Pflicht, die Pakete der Reihe nach zu nehmen; die Leiter ist eine Empfehlung.",

 # ---- the two pairs ----
 "Inside the course": "Im Kurs",
 "The two layers English is made of": "Die beiden Schichten, aus denen Englisch besteht",
 "German Roots and Norman Roots are the pair inside the course.":
   "German Roots und Norman Roots sind das Paar im Kurs.",
 "They are the two layers English is stitched from &mdash; the one it kept from before 1066, and the one that landed with the Conquest &mdash; and they sit between":
   "Sie sind die beiden Schichten, aus denen das Englische genäht ist &mdash; die eine hat es aus der Zeit vor 1066 behalten, die andere kam mit der Eroberung &mdash; und sie liegen zwischen",
 "</b> and <b>English Adept</b>.": "</b> und <b>English Adept</b>.",
 "They are the reason English has": "Sie sind der Grund, warum es im Englischen",
 ". At the end of German Roots you see the seam in a compound word and read the meaning off it. At the end of Norman Roots you read the second layer of English as a layer, not as a list of hard words.":
   " gibt. Am Ende von German Roots siehst du die Naht in einem zusammengesetzten Wort und liest die Bedeutung daran ab. Am Ende von Norman Roots liest du die zweite Schicht des Englischen als Schicht, nicht als Liste schwieriger Wörter.",
 "Step 2 &middot; 1,000 cards &middot; 47 clusters &middot; 48 lessons":
   "Stufe 2 &middot; 1.000 Karten &middot; 47 Cluster &middot; 48 Lektionen",
 "The Germanic backbone of everyday English &mdash; the words you already half-know. Where a Germanic speaker begins, and the default for everyone else.":
   "Das germanische Rückgrat des Alltagsenglischen &mdash; die Wörter, die du halb schon kennst. Hier beginnt, wer eine germanische Sprache spricht, und für alle anderen ist es der Normalfall.",
 "Learn more &rarr;": "Mehr erfahren &rarr;",
 "Step 3 &middot; 1,000 cards &middot; 48 clusters &middot; 49 lessons":
   "Stufe 3 &middot; 1.000 Karten &middot; 48 Cluster &middot; 49 Lektionen",
 "The second English &mdash; 1066 and the French that came with it. Where a Romance speaker begins.":
   "Das zweite Englisch &mdash; 1066 und das Französisch, das damit kam. Hier beginnt, wer eine romanische Sprache spricht.",

 "Beyond the course": "Über den Kurs hinaus",
 "Past the ceiling, for the curious": "Jenseits der Decke, für die Neugierigen",
 "Latin Roots and Greek Roots are the pair beyond the course.":
   "Latin Roots und Greek Roots sind das Paar über dem Kurs.",
 "They come after": "Sie kommen nach",
 ", for readers who want the scholarly layer and the vocabulary of the sciences. A hobby for the linguistically curious, and none the worse for it.":
   ", für Lesende, die die gelehrte Schicht und den Wortschatz der Wissenschaften wollen. Ein Steckenpferd für sprachlich Neugierige, und keinen Deut schlechter dafür.",
 "Part of the course, and past the point where anyone needs them. At the end of the pair an unfamiliar technical or literary word is not unfamiliar: you take it apart on sight, and the boundaries between European languages have started to look like dialect boundaries.":
   "Teil des Kurses und jenseits dessen, was irgendjemand braucht. Am Ende dieses Paares ist ein unbekanntes Fach- oder literarisches Wort nicht mehr unbekannt: Du zerlegst es auf den ersten Blick, und die Grenzen zwischen den europäischen Sprachen fangen an, wie Dialektgrenzen auszusehen.",
 "Step 5 &middot; 500 cards = 1,000 words &middot; 30 clusters &middot; 31 lessons":
   "Stufe 5 &middot; 500 Karten = 1.000 Wörter &middot; 30 Cluster &middot; 31 Lektionen",
 "English built from its parts &mdash; the classical and scientific layer, assembled. Every card teaches a paired cousin word beside the headword.":
   "Englisch aus seinen Bestandteilen gebaut &mdash; die klassische und wissenschaftliche Schicht, zusammengesetzt. Jede Karte lehrt neben dem Stichwort ein verwandtes Partnerwort.",
 "Step 6 &middot; 1,000 cards &middot; 56 clusters &middot; 62 lessons":
   "Stufe 6 &middot; 1.000 Karten &middot; 56 Cluster &middot; 62 Lektionen",
 "The vocabulary of science, medicine, and abstraction &mdash; the layer above Latin, with the Greek script on the card.":
   "Der Wortschatz von Wissenschaft, Medizin und Abstraktion &mdash; die Schicht über dem Lateinischen, mit griechischer Schrift auf der Karte.",
 "Latin Roots and Greek Roots come after Adept, in whichever order you like.":
   "Latin Roots und Greek Roots kommen nach Adept, in der Reihenfolge, die dir gefällt.",

 # ---- the table ----
 "Step by step": "Stufe für Stufe",
 "What you get": "Was du bekommst",
 "Counted from the pack trees, not from copy. Five tiers in every pack. A boss fight at the end of every cluster.":
   "Aus den Paketen selbst gezählt, nicht aus Werbetexten. Fünf Stufen in jedem Paket. Ein Bosskampf am Ende jedes Clusters.",
 ">Step<": ">Stufe<",
 ">Cards<": ">Karten<",
 ">Words<": ">Wörter<",
 ">Clusters<": ">Cluster<",
 ">Lessons<": ">Lektionen<",
 ">Ships as<": ">Ausgeliefert als<",
 ">base game<": ">Grundspiel<",
 "Latin Roots is 500 cards because every one of the 500 teaches a paired cousin word beside the headword: 1,000 words. Where a step shows fewer lessons than clusters (Advance, Adept), its lessons are chapter openings rather than a door on every cluster; the four Roots packs have a lesson at every cluster door, and lessons there are mandatory.":
   "Latin Roots hat 500 Karten, weil jede einzelne der 500 neben dem Stichwort ein verwandtes Partnerwort lehrt: 1.000 Wörter. Wo eine Stufe weniger Lektionen als Cluster zeigt (Advance, Adept), sind ihre Lektionen Kapitelanfänge und keine Tür an jedem Cluster; die vier Roots-Pakete haben an jeder Clustertür eine Lektion, und dort sind sie verpflichtend.",

 # ---- the card / the play ----
 "On every card, in every step": "Auf jeder Karte, auf jeder Stufe",
 "The same card anatomy from the floor to the last Greek root.":
   "Dieselbe Kartenanatomie vom Boden bis zur letzten griechischen Wurzel.",
 "The card": "Die Karte",
 "The play": "Das Spiel",
 "boss fight": "Bosskampf",
 "at the end of every cluster &mdash; eight headwords on the grid, answered on the numberpad":
   "am Ende jedes Clusters &mdash; acht Stichwörter im Raster, beantwortet auf dem Nummernblock",
 "Fibonacci spaced repetition": "Verteilte Wiederholung nach Fibonacci",
 "between the fights": "zwischen den Kämpfen",
 "lesson at every cluster door": "Lektion an jeder Clustertür",
 "in all four Roots packs": "in allen vier Roots-Paketen",
 "<li>three <b>": "<li>drei <b>",
 "revision drills": "Wiederholungsübungen",
 "per pack: the roots rack, antonyms and cloze in German, Latin and Greek Roots; dictation in place of the rack for Norman Roots (its cards are single Old French words, not sums) and for Adept":
   "pro Paket: das Wurzelregal, Antonyme und Lückentext bei German, Latin und Greek Roots; Diktat statt Regal bei Norman Roots (dessen Karten einzelne altfranzösische Wörter sind, keine Summen) und bei Adept",

 # ---- the fight ----
 "The gate": "Das Tor",
 "Every cluster ends in a fight": "Jedes Cluster endet in einem Kampf",
 "Eight headwords on the grid, answered on the numberpad. A right answer moves you one step along the track.":
   "Acht Stichwörter im Raster, beantwortet auf dem Nummernblock. Eine richtige Antwort rückt dich einen Schritt auf der Bahn vor.",
 "A wrong one puts you": "Eine falsche wirft dich",
 "two steps back": "zwei Schritte zurück",
 ", and the cards you are worst at are the ones waiting at positions three and four — so a cluster cannot be beaten until its hardest words are.":
   ", und ausgerechnet die Karten, die du am schlechtesten kannst, warten auf den Positionen drei und vier &mdash; ein Cluster lässt sich also nicht besiegen, bevor seine schwersten Wörter es sind.",
 "Beat it and those cards leave your daily deck for good.":
   "Besiege es, und diese Karten verlassen dein tägliches Deck für immer.",
 "Fight one yourself &rarr;": "Kämpfe selbst einen &rarr;",
 "try the demo": "Demo testen",

 # ---- the law: S3 and S6 verbatim from the ratified German bundle copy ----
 "The immersion law": "Das Immersionsgesetz",
 "English, whatever the menus say": "Englisch, ganz gleich was die Menüs sagen",
 "Card text is English whatever the interface language.":
   "Der Kartentext ist Englisch, ganz gleich welche Oberflächensprache eingestellt ist.",
 "The interface, the cluster names and the Advance lessons are translated into German, Spanish, Japanese, Russian and Chinese. The cards never are &mdash; not one card in any of the six packs carries a foreign-language field. That is what the word":
   "Die Oberfläche, die Clusternamen und die Advance-Lektionen sind ins Deutsche, Spanische, Japanische, Russische und Chinesische übersetzt. Die Karten nie &mdash; keine einzige Karte in einem der sechs Pakete trägt ein fremdsprachiges Feld. Genau das leistet das Wort",
 "is doing here.": "hier.",
 "Every pack carries the British/American spelling split as a layer: a British twin of the headword, both example sentences, the definition and the notes, wherever the British text differs from the American, chosen by your spelling preference or your English voice.":
   "Jedes Paket trägt die Trennung zwischen britischer und amerikanischer Schreibweise als eigene Ebene: ein britischer Zwilling des Stichworts, beider Beispielsätze, der Definition und der Notizen, überall dort, wo sich der britische Text vom amerikanischen unterscheidet, gesteuert von deiner Schreibpräferenz oder deiner englischen Stimme.",

 # ---- the series ----
 "The full immersion series": "Die vollständige Immersionsreihe",
 "Six steps, one series": "Sechs Stufen, eine Reihe",
 "The six steps are one series: the four Roots DLCs &mdash; German and Norman inside the course, Latin and Greek beyond it &mdash; together with the base game that carries English Advance and English Adept. Taken as the series, the floor, the two layers of English, the ceiling and the two Roots past it arrive as a single course rather than six separate packs. Available on Steam.":
   "Die sechs Stufen sind eine Reihe: die vier Roots-DLCs &mdash; German und Norman im Kurs, Latin und Greek darüber hinaus &mdash; zusammen mit dem Grundspiel, das English Advance und English Adept trägt. Als Reihe genommen, kommen der Boden, die beiden Schichten des Englischen, die Decke und die zwei Roots dahinter als ein einziger Kurs statt als sechs getrennte Pakete. Auf Steam erhältlich.",
 "FlashBoss Roots — The Complete Series": "FlashBoss Wurzeln — Die komplette Reihe",
 "is the five items in one: the base game, which\n      carries English Advance and English Adept inside it free, and the four Roots packs. Already own part of\n      it? Steam charges you only for the rest.":
   "sind die fünf Artikel in einem: das Grundspiel, das English Advance und English Adept kostenlos in sich trägt, und die vier Roots-Pakete. Du besitzt schon einen Teil davon? Steam berechnet dir nur den Rest.",
 "The Complete Series &rarr;": "Die komplette Reihe &rarr;",
 "FlashBoss &middot; base game": "FlashBoss &middot; Grundspiel",
 "Start where the ladder says.": "Fang dort an, wo die Leiter es sagt.",
 "The floor and the ceiling ship free with the game. The four Roots packs are on Steam, together or one at a time.":
   "Der Boden und die Decke liegen dem Spiel kostenlos bei. Die vier Roots-Pakete gibt es auf Steam, zusammen oder einzeln.",
 "Knowledge is power": "Wissen ist Macht",

 # ---- the card list: whole <li> each, so no English article survives ----
 "<li>the <b>headword</b>, in English</li>": "<li>das <b>Stichwort</b>, auf Englisch</li>",
 "<li>a <b>definition</b>, in English &mdash; never a translation</li>":
   "<li>eine <b>Definition</b>, auf Englisch &mdash; nie eine Übersetzung</li>",
 "<li>an <b>example sentence</b></li>": "<li>ein <b>Beispielsatz</b></li>",
 "<li>a <b>second example</b> &mdash; the same word again, or a related word, the cousin or twin that the notes go on to explain</li>":
   "<li>ein <b>zweites Beispiel</b> &mdash; dasselbe Wort noch einmal oder ein verwandtes, der Vetter oder Zwilling, den die Notizen dann erklären</li>",
 "<li>a <b>Notes</b> line: part of speech, the parts the word is built from, the etymon</li>":
   "<li>eine <b>Notes</b>-Zeile: Wortart, die Bestandteile, aus denen das Wort gebaut ist, der Etymon</li>",
 "<li><b>synonyms</b> on every card of Adept, German Roots, Norman Roots and Greek Roots; antonyms where a true one exists</li>":
   "<li><b>Synonyme</b> auf jeder Karte von Adept, German Roots, Norman Roots und Greek Roots; Antonyme, wo es ein echtes gibt</li>",
 "<li>the <b>British/American spelling split</b>, as a layer &mdash; see below</li>":
   "<li>die <b>Trennung zwischen britischer und amerikanischer Schreibweise</b>, als eigene Ebene &mdash; siehe unten</li>",

 # ---- fragments that sit between <em> pairs ----
 "</em> and <em>": "</em> und <em>",
 "</em> / Germanic <em>": "</em> / germanisch <em>",
 "</em> / Latin <em>": "</em> / lateinisch <em>",
 " &mdash; the roots, the number and size prefixes, the ":
   " &mdash; die Wurzeln, die Zahl- und Größenpräfixe, die ",
 # ---- the fight GIF's alt: the German capture, so it says what is on screen ----
 "A Latin Roots boss fight in the real game: the boss asks which word means a glass tank for fish, the answer aquarium lands, and the cluster is conquered \u2014 its eight words leaving rotation.":
   "Ein Latin-Roots-Bosskampf im echten Spiel: Die Oberfl\u00e4che ist deutsch, die Karte englisch. Der Boss fragt, welches Wort ein Glasbecken f\u00fcr Fische bezeichnet, die Antwort aquarium sitzt, und das Cluster ist bezwungen \u2014 seine acht W\u00f6rter verlassen den Umlauf.",

 # the two <li>s whose leading article no key covered: dropping it matches the
 # other bullets, which carry no article in any locale.
 "<li>a <b>": "<li><b>",
}
